...
type Config struct {
	...
	// InClusterContextName is the name that Headlamp will use for the in-cluster
	// Kubernetes context when running with in-cluster access enabled.
	// This affects how the cluster shows up in the UI (default: "main").
...
func addGeneralFlags(f *flag.FlagSet) {
	...
	f.String(
		...
		"main",
		"Name to use for the in-cluster Kubernetes context (only meaningful with --in-cluster)",
	)
